...
async function readPackageJson(
  filePath: string
): Promise<Result<PackageJson, string>> {
  try {
 ...
 */
async function getPackagePaths(): Promise<Result<string[], string>> {
  try {
...
async function bumpPackageVersion(
  ...
  Result<{ path: string; oldVersion: string; newVersion: string }, string>
> {
 ...
 */
async function createCommitAndPR(
  version: string,
  bumpType: BumpType,
  dryRun: boolean
): Promise<void> {
...
async function main() {
  ...
  if (!bumpType || !["major", "minor", "patch"].includes(bumpType)) {
    console.error(
      "Usage: bun scripts/bump-version.ts <major|minor|patch> [--dry-run]"
    );
    process.exit(1);
  }
