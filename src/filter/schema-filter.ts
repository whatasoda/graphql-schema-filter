...
export function buildFilteredSchema(
   ...
   */
  function replaceTypeReferences(type: GraphQLType): GraphQLType {
    if (isNonNullType(type)) {
      return new GraphQLNonNull(replaceTypeReferences(type.ofType));
    }

    if (isListType(type)) {
      return new GraphQLList(replaceTypeReferences(type.ofType));
    }
  ...
  function convertFieldToConfig(
    ...
    for (const arg of field.args) {
      args[arg.name] = {
        type: replaceTypeReferences(arg.type),
    ...
    return {
      type: replaceTypeReferences(field.type),
  ...
  function filterObjectFields(
    ...
    for (const [fieldName, field] of Object.entries(fields)) {
      ...
      if (shouldInclude) {
        filteredFields[fieldName] = convertFieldToConfig(field);
      }
    }
   ...
   */
  function filterInputObjectType(
    type: GraphQLInputObjectType
  ): GraphQLInputObjectType | null {
    const filteredFields = filterInputObjectFields(type);
   ...
   * Interface 型をフィルタリング
   */
  function filterInterfaceType(
    type: GraphQLInterfaceType
  ): GraphQLInterfaceType | null {
    const filteredFields = filterInterfaceFields(type);
   ...
   * Object 型をフィルタリング
   */
  function filterObjectType(type: GraphQLObjectType): GraphQLObjectType | null {
    const filteredFields = filterObjectFields(type);
   ...
   * 型をフィルタリング
   */
  function filterType(type: GraphQLNamedType): GraphQLNamedType | null {
    if (TypeKind.isObject(type)) {
      return filterObjectType(type);
    } else if (TypeKind.isInterface(type)) {
      return filterInterfaceType(type);
    } else if (TypeKind.isUnion(type)) {
      return type; // Union はそのまま
    } else if (TypeKind.isInputObject(type)) {
      return filterInputObjectType(type);
   ...
   * フィルタリングされた型のマップを構築
   */
  function buildFilteredTypeMap(): void {
    ...
    for (const [typeName, type] of Object.entries(typeMap)) {
      ...
      if (TypeKind.isInputObject(type)) {
        const filteredInputType = filterInputObjectType(type);
      ...
      const filteredType = filterType(type);
      if (filteredType) {
  ...
  function buildRootType(
    ...
    // フィールドをフィルタリング
    const filteredFields = filterObjectFields(rootType);
  ...
  // Pass 2: ルート型を構築（フィルタリングされた型への参照を使用）
  const queryType = buildRootType("Query");
  const mutationType = buildRootType("Mutation");
  const subscriptionType = buildRootType("Subscription");
